...
**Функции админки товаров** (`/admin/catalog/product/`):
- Список отображает: название, SKU, категорию, бренд, цены, скидку %, флаги, количество вариаций
- Редактирование прямо в списке: цена, старая цена, флаги (популярное, хит, новинка, распродажа, активность)
- Фильтрация по: активности, флагам, категории, бренду
- Поиск по: названию, slug, SKU, описанию

**Массовые операции с товарами** (выбрать товары чекбоксами → Action):
- Отметить/снять флаг "Популярное"
- Отметить/снять флаг "Хит продаж"
- Отметить/снять флаг "Новинка"
- Отметить/снять флаг "Распродажа"
- Сделать активными/неактивными
- Применить скидку (% или фиксированная цена) — заполнить поле над списком
- Сбросить скидку (вернуть old_price → price)
- **Перенести в другую категорию** — выбрать категорию в выпадающем списке над таблицей
