...
export default function Prescriptions() {
  ...
  const inputBase =
    "py-2.5 px-3.5 border rounded-lg text-[15px] transition-colors duration-200 focus:outline-none focus:border-[var(--primary)]";
  const labelBase = "text-[13px] font-semibold";
  const sectionTitle =
    "text-[14px] font-semibold uppercase m-0 mb-4";
  ...
  function buildPdText(p) {
    const parts = [];
    if (p?.pd) parts.push(`общий: ${p.pd} мм`);
    if (p?.pd_right) parts.push(`правый: ${p.pd_right} мм`);
    if (p?.pd_left) parts.push(`левый: ${p.pd_left} мм`);
      ...
      <div className="max-w-[1600px] mx-auto">
        <div className="flex justify-between items-start gap-5 mb-8 md:flex-col md:items-stretch">
          <div>
            ...
            <h1 className="text-[32px] text-center md:text-[26px] font-bold m-0 mb-2" style={{ color: "var(--text)" }}>
              Мои рецепты
            </h1>
        ...
        {prescriptions.length === 0 ? (
          ...
          >
            ...
            <h2 className="text-[20px] font-semibold m-0 mb-2" style={{ color: "var(--text)" }}>
            ...
            {prescriptions.map((p) => (
              ...
              >
                <div className="flex justify-between items-start p-5 pb-0">
                  <div>
                    <h3 className="text-[18px] font-semibold m-0 mb-1 flex items-center gap-2" style={{ color: "var(--text)" }}>
                ...
                {/* Компактный блок: 2 колонки (OD/OS) + PD + Инфо */}
                <div className="p-5">
                  <div className="grid grid-cols-2 md:grid-cols-1 gap-4">
                    <div className="rounded-xl p-4" style={{ background: "var(--bg)" }}>
                      <span className="block text-[12px] font-semibold uppercase mb-2" style={{ color: "var(--muted)" }}>
                        OD (правый глаз)
                      </span>
                      <span className="text-[15px] font-medium font-mono" style={{ color: "var(--text)" }}>
                        {p.od_display}
                      </span>
                    </div>
                    ...
                    <div className="rounded-xl p-4" style={{ background: "var(--bg)" }}>
                      ...
                      <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-[13px]" style={{ color: "var(--text)" }}>
                        <div className="min-w-0">
                          <span className="block text-[12px]" style={{ color: "var(--muted)" }}>
                            Осмотр
                          </span>
                          <span className="block truncate">{p.exam_date || "—"}</span>
                        </div>
                        ...
                        <div className="min-w-0">
                          ...
                          <span className="block truncate">{p.expiry_date || "—"}</span>
                        </div>
                        ...
                        <div className="min-w-0">
                          ...
                          <span className="block truncate">{p.doctor_name || "—"}</span>
                        </div>
                        ...
                        <div className="min-w-0">
                          ...
                          <span className="block truncate">{p.clinic_name || "—"}</span>
                        </div>
                      </div>

                      {p.notes ? (
                        <div className="mt-3 text-[13px]" style={{ color: "var(--muted)" }}>
                          <span className="block text-[12px] font-semibold uppercase mb-1" style={{ color: "var(--muted)" }}>
                            Примечания
                          </span>
                          <span className="block line-clamp-2">{p.notes}</span>
                        </div>
                      ) : null}
                    </div>
                  </div>
        ...
        {showForm && (
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[1000] p-5" onClick={closeForm}>
            <div
              className="rounded-2xl w-full max-w-[980px] max-h-[90vh] overflow-hidden flex flex-col md:max-h-full md:rounded-none"
              style={{ background: "var(--card)" }}
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex justify-between items-center py-5 px-6 border-b" style={{ borderColor: "var(--border)" }}>
                <h2 className="text-[20px] font-semibold m-0">{editingId ? "Редактировать рецепт" : "Новый рецепт"}</h2>
                <button
              ...
              <form className="overflow-y-auto p-6" onSubmit={handleSubmit}>
                {/* Основная информация */}
                <div className="mb-8">
                  <h3 className={sectionTitle} style={{ color: "var(--muted)" }}>
                    Основная информация
                  </h3>

                  <div className="grid grid-cols-2 md:grid-cols-1 gap-4">
                    <div className="flex flex-col gap-1.5">
                      <label className={labelBase} style={{ color: "var(--muted)" }}>
                        Название
                      </label>
                      <input
                        type="text"
                ...
                <div className="mb-8">
                  <h3 className={sectionTitle} style={{ color: "var(--muted)" }}>
                    PD и информация о рецепте
                  </h3>

                  <div className="grid grid-cols-2 md:grid-cols-1 gap-5">
                    {/* PD */}
                    <div className="rounded-2xl border p-5" style={{ borderColor: "var(--border)", background: "var(--bg)" }}>
                      <div className="text-[14px] font-semibold mb-3" style={{ color: "var(--text)" }}>
                        Межзрачковое расстояние (PD)
                      </div>
                  ...
                  {/* Notes + Primary (на всю ширину) */}
                  <div className="mt-5 rounded-2xl border p-5" style={{ borderColor: "var(--border)", background: "var(--bg)" }}>
                    <div className="grid grid-cols-2 md:grid-cols-1 gap-4 items-start">
                      ...
                      <div className="flex items-start justify-start md:pt-1">
                        <label className="flex items-center gap-2.5 cursor-pointer select-none">
                          <input
                            ...
                            onChange={handleChange}
                            className="w-4.5 h-4.5 accent-[var(--primary)]"
                          />
                          <span className="text-[14px]" style={{ color: "var(--text)" }}>
                            Использовать как основной рецепт
                          </span>
                        </label>
                      </div>
                    </div>
                ...
                <div className="flex justify-end gap-3 pt-4 border-t md:flex-col" style={{ borderColor: "var(--border)" }}>
                  <button
                    type="button"
